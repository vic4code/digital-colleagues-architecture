...
![主動工作的高層流程](proactive-high-level.svg)

- 給決策者：[互動概念與三種結果](show-me-proactive-lifecycle.html#proactive-overview)。
- 給設計者：[觸發與進階機制詳細查核](source-notes/proactive-trigger-mechanisms.md)。
- 另一條主線：[同事互動感](colleague-interaction.md)，研究接話、修正、群組發言與交接。
- 歷史補正：[OpenClaw inferred commitments 已退役](source-notes/openclaw-inferred-commitments.md)；[工作閉環共用設計](source-notes/colleague-experience.md)。
...
- **接續：** 模型可決定動態查看間隔，排程再喚起下一回合；各模式存活範圍不同。2026-09-11 官方 `/goal` 文件另描述每輪後由小模型評估完成條件，未完成再接續；詳見 [本次補查](source-notes/proactive-trigger-mechanisms.md)。
