...
function create(details,scene){
 ...
 root.innerHTML=`<div class="proactive-comic"><header class="comic-heading"><strong data-role="name"></strong><button type="button" data-role="close">Close / 收起</button></header><div class="comic-stage" data-step="0"><div class="comic-person">${avatar}<span class="comic-small">Digital colleague<span lang="zh-Hant">數位同事</span></span></div><div class="comic-bubble" data-role="speech" aria-live="polite" aria-atomic="true"></div></div><div class="comic-navigation"><ol class="comic-frames" data-role="frames" aria-label="Scenario frames / 情境分鏡"></ol><span class="comic-count" data-role="count"></span></div><div class="comic-controls"><button class="comic-play" type="button" data-role="play">Play / 播放</button><button type="button" data-role="prev" aria-label="Previous frame / 上一格">←</button><button type="button" data-role="next" aria-label="Next frame / 下一格">→</button><button type="button" data-role="replay">Restart / 重播</button><a class="comic-evidence" data-role="evidence">Implementation / 看實作 ↗</a></div><footer class="comic-scope" data-role="scope"></footer></div>`;
 ...
 function render(){
  ...
  el('frames').replaceChildren(...scene.frames.map((f,i)=>{const li=document.createElement('li');const b=document.createElement('button');b.type='button';b.textContent=String(i+1).padStart(2,"0");b.title=f.join(" / ");b.setAttribute("aria-label",`Frame ${i+1}: ${f[0]} ${f[1]}`);b.setAttribute('aria-current',String(i===step));b.addEventListener('click',()=>{pause();step=i;render();el('frames').querySelectorAll('button')[i].focus({preventScroll:true});});li.append(b);return li;}));
